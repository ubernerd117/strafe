...
The interactive preview shows three captured app screens for the example search “neo-tree go up a directory”: reader view, raw view, and Brave AI overview. Click the preview to use h/l, j/k, 1–3, slash, and Escape; use view tabs or arrow buttons on touch devices. Each view links to its full-size screenshot. The fixed search only resets the preview and makes no API calls. The AI overview is a recorded example, not a generated answer. Download links open the project's latest GitHub release.
...
`assets/strafe-demo.mp4` is the 37-second recording shown below the hero. The player uses native controls, plays inline on mobile, and waits for playback before loading the video. `assets/search-result-1.png` supplies its preview and the root README's clickable thumbnail; the raw-view and AI-overview PNGs supply the other preview tabs.
